...
<div class="ui-console-page">
    ...
    <form method="POST" action="{{ route('admin.usuarios.store') }}" class="mt-5 space-y-4">
        ...
        <x-dashboard.section-card title="Dados básicos" subtitle="Informações principais da conta">
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input id="name" name="name" type="text" value="{{ old('name') }}" class="ui-form-control" required>
                    @error('name')<p class="ui-form-error">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label class="ui-form-label" for="email">E-mail (opcional)</label>
                    <input id="email" name="email" type="email" value="{{ old('email') }}" class="ui-form-control" placeholder="ex: pessoa@dominio.com">
                    @error('email')<p class="ui-form-error">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label class="ui-form-label" for="cpf_mask">CPF (opcional)</label>
                    <input id="cpf_mask" type="text" inputmode="numeric" placeholder="000.000.000-00" value="{{ old('cpf') ? preg_replace('/(\d{3})(\d{3})(\d{3})(\d{2})/','$1.$2.$3-$4',preg_replace('/\D+/','',old('cpf'))) : '' }}" class="ui-form-control">
        ...
        <x-dashboard.section-card title="Acesso e segurança" subtitle="Defina a senha inicial da conta">
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input id="password" name="password" type="password" class="ui-form-control" required>
                    @error('password')<p class="ui-form-error">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label class="ui-form-label" for="password_confirmation">Confirmar senha</label>
                    <input id="password_confirmation" name="password_confirmation" type="password" class="ui-form-control" required>
                </div>
            </div>
        </x-dashboard.section-card>
        ...
        <x-dashboard.section-card title="Papéis" subtitle="Selecione apenas um papel">
            ...
            <select name="roles[]" class="ui-form-select">
                        ...
                        <div class="grid gap-2 sm:grid-cols-2">
                            @foreach($perms as $perm)
                                ...
                                <label for="{{ $pid }}" class="inline-flex items-center gap-2">
                                    <input id="{{ $pid }}" type="checkbox" name="perms[]" value="{{ $perm->name }}" class="perm-chk ui-form-check rounded" data-group="{{ $group }}" @checked(in_array($perm->name,$oldPerms))>
