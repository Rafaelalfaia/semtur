...
<div class="mx-auto w-full max-w-[1100px] px-4 md:px-6 py-6 md:py-10 space-y-6">
  ...
  <form method="POST" action="{{ route('admin.usuarios.update',$usuario) }}" class="space-y-6">
      ...
      <div class="grid md:grid-cols-2 gap-4">
        <div>
          ...
          <input id="cpf_mask" type="text" inputmode="numeric" placeholder="000.000.000-00"
                 value="{{ old('cpf', $maskCpf($usuario->cpf)) }}"
                 class="w-full rounded-lg bg-white/5 border border-white/10 px-3 py-2">
    ...
    <section class="rounded-xl border border-white/10 bg-[#0F1412] p-4 md:p-5 space-y-4">
      ...
      <div class="grid md:grid-cols-2 gap-4">
        <div>
          <label class="block text-sm text-slate-300 mb-1" for="password">Nova senha (opcional)</label>
          <input id="password" name="password" type="password"
                 class="w-full rounded-lg bg-white/5 border border-white/10 px-3 py-2">
          @error('password')<p class="text-xs text-rose-300 mt-1">{{ $message }}</p>@enderror
        </div>
        <div>
          <label class="block text-sm text-slate-300 mb-1" for="password_confirmation">Confirmar nova senha</label>
    ...
    <div class="flex items-center justify-between">
        <h2 class="text-sm font-semibold tracking-wide text-slate-200">Permissões (refino opcional)</h2>
        ...
        <fieldset class="rounded-lg border border-white/5 bg-white/0 p-3">
            ...
            @foreach($perms as $perm)
                ...
                <input id="{{ $pid }}" type="checkbox" name="perms[]"
                        ...
                        data-group="{{ $group }}"
                        @checked(in_array($perm->name,$oldPerms,true))>
    ...
    <div class="flex items-center justify-between gap-3">
      <a href="{{ route('admin.usuarios.index') }}"
         class="rounded-lg border border-white/10 bg-white/5 hover:bg-white/10 px-4 py-2">Voltar</a>
      ...
      <div class="flex items-center gap-3">
        @if(auth()->id() !== $usuario->id)
          <form method="POST" action="{{ route('admin.usuarios.destroy',$usuario) }}"
            ...
            @csrf @method('DELETE')
            <button type="submit"
              class="rounded-lg border border-red-500/30 bg-red-500/10 hover:bg-red-500/20 px-4 py-2">
              Excluir
            </button>
          </form>
        @else
          <button class="rounded-lg border border-white/10 bg-white/5 px-4 py-2 opacity-50 cursor-not-allowed" disabled>
            Excluir
          </button>
        @endif

        <button class="rounded-lg bg-emerald-600 hover:bg-emerald-500 px-5 py-2 font-semibold">
          Salvar alterações
        </button>
      </div>
    </div>
