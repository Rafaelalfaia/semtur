...
<div class="mx-auto w-full max-w-[900px] px-4 md:px-6 py-6 md:py-10 space-y-6">
  ...
  @if(session('ok'))
    <div class="rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-emerald-200">{{ session('ok') }}</div>
  @endif
  @if ($errors->any())
  ...
  <form method="POST" action="{{ route('coordenador.tecnicos.update',$usuario) }}" class="space-y-6">
    @csrf @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
              @foreach($perms as $p)
                ...
                <label class="inline-flex items-center gap-2">
                  <input type="checkbox" name="perms[]" value="{{ $p->name }}" class="h-4 w-4 cb-{{ $group }}" @checked($checked)>
                  {{ $labelAction($action) }}
                </label>
              @endforeach
            </div>
    ...
    <div>
      ...
      <a href="{{ route('coordenador.tecnicos.index') }}" class="ml-2 px-4 py-2 rounded-lg border border-white/10">Voltar</a>
    </div>
  </form>
</div>
