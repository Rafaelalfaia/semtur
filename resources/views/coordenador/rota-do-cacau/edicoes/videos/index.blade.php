    ...
    <x-slot:actions>
      <div class="flex flex-wrap gap-2">
        @can('rota_do_cacau.edicoes.videos.create')
          <a href="{{ route('coordenador.rota-do-cacau.edicoes.videos.create', [$rota, $edicao]) }}" class="ui-btn-primary">Novo vídeo</a>
        @endcan
        <a href="{{ route('coordenador.rota-do-cacau.edicoes.edit', [$rota, $edicao]) }}" class="ui-btn-secondary">Voltar à edição</a>
      </div>
    </x-slot:actions>
  </x-dashboard.page-header>

  <div class="mt-2 text-sm text-[var(--ui-text-soft)]">Rota do Cacau > Edições > {{ $edicao->titulo }} > Vídeos</div>
          ...
          @forelse($videos as $video)
            <tr class="ui-table-row">
              ...
              <td class="px-3 py-3">
                @canany(['rota_do_cacau.edicoes.videos.update', 'rota_do_cacau.edicoes.videos.delete'])
                  <div class="flex flex-wrap items-center justify-end gap-2">
                    ...
                    @can('rota_do_cacau.edicoes.videos.delete')
                      <form method="POST" action="{{ route('coordenador.rota-do-cacau.edicoes.videos.destroy', [$rota, $edicao, $video]) }}" onsubmit="return confirm('Excluir este vídeo da edição?');">
                        @csrf
                        @method('DELETE')
                ...
                @else
                  <span class="text-sm text-[var(--ui-text-soft)]">Sem ações disponíveis.</span>
                @endcanany
              </td>
            </tr>
          @empty
