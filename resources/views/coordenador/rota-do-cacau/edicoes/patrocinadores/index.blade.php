...
<div class="ui-console-page">
  ...
  <x-dashboard.page-header title="Patrocinadores da edicao" subtitle="Cadastre logo, URL e ordem de exibicao para os apoiadores desta edicao.">
    <x-slot:actions>
      <div class="flex flex-wrap gap-2">
        @can('rota_do_cacau.edicoes.patrocinadores.create')
          <a href="{{ route('coordenador.rota-do-cacau.edicoes.patrocinadores.create', [$rota, $edicao]) }}" class="ui-btn-primary">Novo patrocinador</a>
        @endcan
  ...
  <div class="mt-2 text-sm text-[var(--ui-text-soft)]">Rota do Cacau > Edicoes > {{ $edicao->titulo }} > Patrocinadores</div>
  ...
  <x-dashboard.section-card title="Lista de patrocinadores" subtitle="Os patrocinadores pertencem exclusivamente a esta edicao." class="ui-coord-dashboard-panel mt-5">
      ...
      <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        @foreach($patrocinadores as $patrocinador)
          <article class="rounded-3xl border border-[var(--ui-border)] bg-[var(--ui-surface)] p-4">
            ...
            @canany(['rota_do_cacau.edicoes.patrocinadores.update', 'rota_do_cacau.edicoes.patrocinadores.delete'])
              <div class="mt-4 flex flex-wrap gap-2">
                @can('rota_do_cacau.edicoes.patrocinadores.update')
                  <a href="{{ route('coordenador.rota-do-cacau.edicoes.patrocinadores.edit', [$rota, $edicao, $patrocinador]) }}" class="ui-btn-secondary">Editar</a>
                @endcan
                @can('rota_do_cacau.edicoes.patrocinadores.delete')
                  <form method="POST" action="{{ route('coordenador.rota-do-cacau.edicoes.patrocinadores.destroy', [$rota, $edicao, $patrocinador]) }}" onsubmit="return confirm('Excluir este patrocinador?');">
                    @csrf
                    @method('DELETE')
    ...
    @else
      <div class="rounded-3xl border border-dashed border-[var(--ui-border)] bg-[var(--ui-surface-soft)] p-8 text-center">
        ...
        @can('rota_do_cacau.edicoes.patrocinadores.create')
          <a href="{{ route('coordenador.rota-do-cacau.edicoes.patrocinadores.create', [$rota, $edicao]) }}" class="ui-btn-primary mt-5 inline-flex">Adicionar patrocinador</a>
        @endcan
      </div>
    @endif
