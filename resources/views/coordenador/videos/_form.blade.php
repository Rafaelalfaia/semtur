@php
    ...
    $coverCurrent = old('remover_capa') ? null : ($video->capa_url ?? null);
@endphp
...
<div
    x-data="{
        ...
        linkAcesso: @js(old('link_acesso', $video->link_acesso ?? '')),
...
>
    ...
    <section class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div class="space-y-6">
            ...
            <section class="rounded-2xl border border-white/10 bg-[#0F1412] p-5">
                <div class="mb-5">
                    <div class="text-xs uppercase tracking-[0.18em] text-sky-300/80">Acesso</div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            name="link_acesso"
                            x-model="linkAcesso"
                            value="{{ old('link_acesso', $video->link_acesso ?? '') }}"
                            class="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-slate-100"
                    ...
                    <div class="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                        ...
                        <div class="mt-3 flex flex-wrap gap-2">
                            <a
                                ...
                                class="inline-flex items-center rounded-xl bg-sky-600 px-4 py-2 text-sm font-medium text-white hover:bg-sky-700"
                            >
                                Testar preview
                            </a>
                ...
                <div>
                    ...
                    >
                        @foreach(($statuses ?? []) as $itemStatus)
                            <option value="{{ $itemStatus }}" @selected(old('status', $video->status ?? 'rascunho') === $itemStatus)>
                                {{ ucfirst($itemStatus) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <div class="ui-video-cover-preview mt-4 overflow-hidden rounded-2xl border border-white/10 bg-slate-900/60">
                        <div class="relative h-56 w-full bg-slate-800">
                            ...
                            <template x-if="!coverPreview || removeCurrentCover">
                                <div class="ui-video-cover-preview-fallback absolute inset-0"></div>
                            </template>
                        </div>
                        ...
                        <div class="mt-3">
                            <label class="inline-flex items-center gap-2 text-sm text-slate-300">
                                ...
                                <input
                                    ...
                                    class="rounded border-white/20 bg-white/5 text-sky-500 focus:ring-sky-500"
                                >
                                Remover capa atual
                            </label>
                        </div>
