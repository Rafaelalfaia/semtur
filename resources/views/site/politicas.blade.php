...
@section('title', ui_text('ui.policy.title').' • VisitAltamira')
...
<div class="min-h-dvh bg-white text-slate-900">
  <header class="sticky top-0 z-10 bg-white/80 backdrop-blur border-b border-slate-100">
    <div class="mx-auto max-w-lg px-4 py-3 flex items-center gap-3">
      <a href="{{ url()->previous() ?: localized_route('site.perfil.index') }}"
      ...
      <h1 class="text-lg sm:text-xl font-semibold">{{ ui_text('ui.policy.title') }}</h1>
    </div>
  </header>

  <main class="mx-auto max-w-lg px-6 py-6 pb-24 prose prose-slate prose-sm">
    <h2>1. {{ ui_text('ui.policy.intro_title') }}</h2>
    <p>Esta política descreve como coletamos e tratamos seus dados no aplicativo VisitAltamira.</p>
    ...
    <p class="text-xs text-slate-500 mt-6">{{ ui_text('ui.policy.last_update') }}: {{ now()->format('d/m/Y') }}</p>
  </main>
</div>
