...
@section('title', $title ?? 'Console')
@section('page.title', $title ?? 'Console')
...
<div x-data="{ tab: 'resumo' }" class="space-y-6">
  ...
  {{-- Cards principais (estilo do mock) --}}
  <div class="grid gap-6 lg:grid-cols-2">
    <section class="rounded-xl border border-white/5 bg-[#0F1412] p-5">
      <header class="flex items-center justify-between">
        <div>
          <div class="text-sm text-slate-400">Conteúdo</div>
          <h2 class="text-lg font-semibold">Publicações & Cadastros</h2>
        </div>
  ...
  <div class="grid gap-4 sm:grid-cols-3">
    @foreach ([
      ['label'=>'Categorias','val'=>$stats['categorias'],'link'=>'#'],
      ['label'=>'Empresas','val'=>$stats['empresas'],'link'=>'#'],
      ['label'=>'Pontos Turísticos','val'=>$stats['pontos'],'link'=>'#'],
    ] as $c)
      <div class="rounded-xl border border-white/5 bg-[#0F1412] p-4">
        <div class="text-sm text-slate-400">{{ $c['label'] }}</div>
        <div class="mt-1 text-2xl font-bold">{{ number_format($c['val']) }}</div>
        <a href="{{ $c['link'] }}" class="mt-3 inline-block text-sm text-emerald-300 hover:underline">Ver todos</a>
      </div>
    @endforeach
  </div>
